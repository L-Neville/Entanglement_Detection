...
class DVCircuit():
    ...
    def __init__(self, ifnoise=False, 
        ...
        num_mode = 6 + repeatation * 2 + 2

        self.Qa = [num_mode//2-1, num_mode//2]
        self.Q1 = [num_mode//2-3, num_mode//2-2]
        self.Q2 = [num_mode//2+1, num_mode//2+2]
        ...
        state1[self.Qa[0]] = 1
        state1[self.Q2[0]] = 1
        state1[self.Q1[0]-2] = 1

        state2 = np.zeros(num_mode, dtype=int)
        state2[self.Q1[0]] = 1
        state2[self.Qa[1]] = 1
        state2[self.Q2[1]] = 1
        state2[self.Q1[0]-2] = 1
        ...
        for rep in range(repeatation):
            ...
            last_qumode = num_mode//2+2 + 2 + rep 
            first_qumode = num_mode//2-3 - 2 - rep 

            # swap
            self.cir.mzi(wires=[self.Q1[0], self.Q2[0]], inputs=[0+noise(sigma), 0+noise(sigma)])
            self.cir.ps(wires=self.Q1[0], inputs=-np.pi/2+noise(sigma))
            self.cir.ps(wires=self.Q2[0], inputs=-np.pi/2+noise(sigma))

            self.cir.mzi(wires=[self.Q1[1], self.Q2[1]], inputs=[0+noise(sigma), 0+noise(sigma)])
            self.cir.ps(wires=self.Q1[1], inputs=-np.pi/2+noise(sigma))
            self.cir.ps(wires=self.Q2[1], inputs=-np.pi/2+noise(sigma))

            # collision
            self.E = [first_qumode, self.Q1[0]-1]
            self.e = [self.Q2[1]+1, last_qumode]
            self.cir.mzi(wires=[self.E[1], self.Q1[0]], inputs=[param['MZI1'][0]+noise(sigma), param['MZI1'][1]+noise(sigma)])
            self.cir.mzi(wires=[self.Q2[1], self.e[0]], inputs=[param['MZI2'][0]+noise(sigma), param['MZI2'][1]+noise(sigma)])
            self.cir.mzi(wires=[self.E[0], self.E[1]], inputs=[param['MZI3'][0]+noise(sigma), param['MZI3'][1]+noise(sigma)])
            self.cir.mzi(wires=[self.e[0], self.e[1]], inputs=[param['MZI4'][0]+noise(sigma), param['MZI4'][1]+noise(sigma)])
    ...
    def basis_output_prob(self, basis='iii'):
        ...
        for w_state in wanted_state:
            index = wanted_state.index(w_state)
    ...
    def tomography(self, if_sampling=False, shots=1000):
        ...
        def expct_based_tomography(num_qubtis:int, Pauli_expectations:dict[str, np.float32])->np.ndarray:
            ...
            '''
            self.rho = sum([Pauli(obsv).to_matrix() * expct for obsv, expct in Pauli_expectations.items()]) / 2 ** num_qubtis
            return self.rho
        ...
        def sample_based_tomography(num_qubits:int, samples:dict[str, np.ndarray], output_expct=False)->np.ndarray:
            ...
            '''
            self.parities = np.zeros((64, 8), dtype=int)
